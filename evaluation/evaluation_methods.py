...
def evaluate_all(policies_to_evaluate, evaluation_episodes, generalisation_episodes, USE_ADE=False):
    ...
    if generalisation_episodes is not None:
        for policy_to_evaluate in policies_to_evaluate:
            micro_eval, _ = get_file_name_evaluation(policy_to_evaluate, "dataset_one", irl_weights, generalisation_episodes)
            macro_eval, _ = get_file_name_evaluation(policy_to_evaluate, "dataset_all", irl_weights, generalisation_episodes)
    ...
    for policy_to_evaluate in policies_to_evaluate:
        ...
        for score in overall_micro_analysis_scores[policy_to_evaluate].keys():
            average_scores[score] = (overall_micro_analysis_scores[policy_to_evaluate][score] + overall_macro_analysis_scores[policy_to_evaluate][score]) / 2
       ...
       # If one of the scores is missing, we set the average to NaN
        ...
        if USE_ADE:
            overall_scores[policy_to_evaluate]["S_Fidelity"] = np.nanmean([average_scores["S_FDE"], average_scores["S_ADE"], average_scores["S_TD_ADE"]])
        else:
        ...
        overall_scores[policy_to_evaluate]["S_Safety"] = np.nanmean([average_scores["S_Collision"], average_scores["S_Off_road"], average_scores["S_Distance_nearby"]])
        ...
        if generalisation_episodes is not None:
            overall_scores[policy_to_evaluate]["S_Diversity"] = np.nanmean([average_scores["S_Distance_right_marking"], average_scores["S_Generalisation"]])
        else:
        ...
        overall_scores[policy_to_evaluate]["S_Comfort"] = np.nanmean([average_scores["S_TTC"], average_scores["S_TTH"], average_scores["S_Velocity"], average_scores["S_Interference"]])
...
def generate_latex_table(results, map):
    ...
    def latex(df, type, map, metrics, caption=None):
        ...
        latex_table = df.to_latex(index=True, header=True, column_format='||c | c | c | c | c||', caption=caption, float_format="%.2f", label=label)
    ...
    print(latex(average_df, type="average", map=map, metrics=True, caption=f"Average metrics for the evaluation in {map}"))
