    ...
    def test_time_steps_equal(self):
        ...
        for agent_id, features in self.__simulation_agent_features.items():
            ...
            try:
                ...
                if features["death_cause"] is None:
                ...
                for position in simulated_nearby_vehicles[step_idx]:
                    if simulated_nearby_vehicles[step_idx][position] is None:
                        if real_nearby_vehicles[step_idx][position.value] is not None:
                            miscounts.append(f"Real nearby vehicles: {real_nearby_vehicles[step_idx][position.value]}, Simulated nearby vehicles: {simulated_nearby_vehicles[step_idx][position]}, at step {step_idx} for agent {agent_id}, position {position}")
                    else:
                        if real_nearby_vehicles[step_idx][position.value] != simulated_nearby_vehicles[step_idx][position].agent_id:
                            miscounts.append(f"Real nearby vehicles: {real_nearby_vehicles[step_idx][position.value]}, Simulated nearby vehicles: {simulated_nearby_vehicles[step_idx][position]}, at step {step_idx} for agent {agent_id}, position {position}")
            ...
            for step_idx in range(len(simulated_ttc)):
                ...
                for position in simulated_ttc[step_idx]:
                    ...
                    if simulated_ttc[step_idx][position] is None or real_ttc[step_idx][position.value] is None:
                        if not (simulated_ttc[step_idx][position] is None and real_ttc[step_idx][position.value] is None):
                            miscounts.append(f"Real TTC: {real_ttc[step_idx][position.value]}, Simulated TTC: {simulated_ttc[step_idx][position]}, at step {step_idx} for agent {agent_id}, position {position}")
                    else:
                    ...
                    if simulated_tth[step_idx][position] is None or real_tth[step_idx][position.value] is None:
                        if not (simulated_tth[step_idx][position] is None and real_tth[step_idx][position.value] is None):
                            miscounts.append(f"Real TTH: {real_tth[step_idx][position.value]}, Simulated TTH: {simulated_tth[step_idx][position]}, at step {step_idx} for agent {agent_id}, position {position}")
        ...
        for agent_id, features in self.__simulation_agent_features.items():
            ...
            for step_idx in range(len(simulated_right_marking_distance)):
                if real_right_marking_distance[step_idx] - simulated_right_marking_distance[step_idx] > 0.001:
                    miscounts.append(f"Real right marking distance: {real_right_marking_distance[step_idx]}, Simulated right marking distance: {simulated_right_marking_distance[step_idx]}, at step {step_idx} for agent {agent_id}")
                if real_left_marking_distance[step_idx] - simulated_left_marking_distance[step_idx] > 0.001:
                    miscounts.append(f"Real left marking distance: {real_left_marking_distance[step_idx]}, Simulated left marking distance: {simulated_left_marking_distance[step_idx]}, at step {step_idx} for agent {agent_id}")
