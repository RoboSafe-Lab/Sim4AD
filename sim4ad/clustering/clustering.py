...
class FeatureExtraction:
    """
    refer to the paper "Classifying travelers’ driving style using basic safety messages generated by connected
    vehicles: Application of unsupervised machine learning"
    """

    def __init__(self):
        self.features = {'episode_id': [], 'agent_id': [], 'speed_vf': [], 'speed_dmean': [], 'speed_cv': [],
    ...
    def extract_features(self, episode):
        ...
        for agent_id, agent in episode.agents.items():
            ...
            self.features['episode_id'].append(episode.config.recording_id)
            self.features['agent_id'].append(agent_id)
            # compute average value for clustering
            self.features["speed_vf"].append(np.average(results_containers[0]))
            self.features["speed_dmean"].append(np.average(results_containers[1]))
            self.features["speed_cv"].append(np.average(results_containers[2]))
            self.features["speed_qcv"].append(np.average(results_containers[3]))
            self.features["accx_dmean"].append(np.average(results_containers[4]))
            self.features["accy_dmean"].append(np.average(results_containers[5]))
...
def main():
    ...
    for episode in data_loader.scenario.episodes:
        ...
        feature_extractor.extract_features(episode=episode)

    df = pd.DataFrame(feature_extractor.features)
    feature_names = list(feature_extractor.features.keys())
