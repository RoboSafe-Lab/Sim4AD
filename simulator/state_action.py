...
class State:
    ...
    def __init__(self, time: float, position: Union[np.ndarray[float, float], Point], velocity: float,
                 acceleration: float, heading: float, agent_length: float, agent_width: float, lane: Lane = None):
        self.time = time
...
class Action:
    ...
    def __init__(self, acceleration: float, yaw_rate: float):
        ...
        :param acceleration: longitudinal acceleration (i.e., acceleration in the direction of the heading)
        :param yaw_rate: rate of change of the heading
        """
        self.__ax = acceleration
